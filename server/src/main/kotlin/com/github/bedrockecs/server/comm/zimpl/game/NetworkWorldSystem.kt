...
class NetworkWorldSystem(
    ...
    override fun tick() {
        ...
        val resolvedPendingPlayers = mutableMapOf<UUID, GameWorldHandler.SpawnedPlayer>()
        db.entities.scan(
            EntityScanConfig(),
            ...
            if (type == PlayerEntityType.TYPE) {
                if (!players.contains(pid.uuid)) {
                    ...
                    resolvedPendingPlayers.put(pid.uuid, GameWorldHandler.SpawnedPlayer(eid, epc))
                }
            }
            playerPositions[pid.uuid] = epc.pos
        }

        handler.resolvePendingPlayers(resolvedPendingPlayers)
